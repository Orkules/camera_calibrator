...
def get_app_root() -> Path:
    """Return the root directory for templates/config regardless of PyInstaller."""
...
def get_frame_route():
    ...
    return jsonify({"frame": encode_frame(frame)})
...
def open_browser_after_start(port: int):
    """Open the default web browser once the Flask server is starting up."""

    def _open():
        url = f"http://127.0.0.1:{port}"
        try:
            webbrowser.open_new(url)
            add_console_message(f"Opened browser at {url}")
        except Exception as exc:
            logging.warning(f"Failed to open browser automatically: {exc}")
